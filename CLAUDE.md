...
- **`search-index.json`** — generated by **`node scripts/build-search-index.js`** (also runs in CI on push to `main` before S3 deploy). Canonical index for **product Home search** and docs CDN; synced to `plg-crm-ui/src/data/docs-search-index.json` separately.
- See **[SEARCH-INDEX.md](./SEARCH-INDEX.md)** for schema, keywords, `deprecated`, and update workflow.
...
- **`scripts/build-search-index.js`** — walks HTML, builds and validates `search-index.json` (run automatically in CI on `main`).
